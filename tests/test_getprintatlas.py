...
def test_no_template(client):
    ...
    assert (
        b["message"]
        == "ATLAS - Error from the user while generating the PDF: TEMPLATE is required"
    )
...
def test_invalid_scale(client):
    ...
    assert (
        b["message"]
        == "ATLAS - Error from the user while generating the PDF: Invalid number in SCALE."
    )
...
def test_invalid_scales(client):
    ...
    assert (
        b["message"]
        == "ATLAS - Error from the user while generating the PDF: Invalid number in SCALES."
    )
