        ...
        for layer in QgsProject.instance().mapLayers().values():
            if layer.dataProvider().name() == 'OAPIF':
                ...
                if uri.hasParam('url') and uri.hasParam('typename'):
                    if uri.param('typename') in ['permits','permits_poly', 'permits_line', 'permits_point',]:
                        ...
                        TODO
                        uri.setParam('url', 'http://localhost:9095/wfs3/?permit_request_id=4')
                        layer.setDataSource(uri.uri(expandAuthConfig=False), uri.param('typename'), "OAPIF",  QgsDataProvider.ProviderOptions())
